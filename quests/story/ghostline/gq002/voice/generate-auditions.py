...
V always uses the supplied cached SafeTensors x-vector. Cinder is intentionally
...
def main() -> int:
    ...
    v_embedding = load_file(str(args.v_embedding))["embedding"].to(clone.device)
    ...
    for label, instruction in CINDER_DESIGNS.items():
        ...
        save_file(
            ...
            str(args.output / label / "cinder.safetensors"),
        )
