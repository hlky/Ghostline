...
This reference lists focused helper commands. Start with
[`../workflows/development.md`](../workflows/development.md) for the normal
development loop.
...
## Braindance Blender Scene Generator
...
Outputs default to ignored `.tmp/braindance` paths. The builder accepts real
actor/environment GLTF assets but generates standalone proxies when none are
declared. GQT005 uses the checked rig-only
`braindance/rigs/man_base.glb` together with
`braindance/rigs/man_base.skeleton.json`. Actors declare that pair with
`asset.path` and `rig.contract`; a `body_animation` of type
`walk_from_root_motion` optionally authors a gait synchronized to their root
keys.

The skeleton sidecar is the source of truth for RED's exact 71-joint order,
parents, rest transforms, and trajectory index. Because Blender can reorder
armature bones when importing a GLB, the bake maps every joint by name into
the sidecar index rather than using Blender's data-bone order.

`tools/braindance_rid.py` then compiles the handoff against a vanilla
`.scenerid` or `.scenerid.json` structural template:
...
body/facial/cyberware armature pose bones, declared facial float properties,
camera transform, and focal length. It then authors RID identity/placement,
emits a deterministic report, performs a WolvenKit binary round trip, and
requires the custom buffer hashes to survive unchanged.

For a real rig-contract actor, the selected Holt/vanilla actor slot supplies
only RED layouts, handle shapes, and joint/track cardinality. The compiler
rebuilds the complete pose buffer from Blender samples and never merges donor
pose channels or compressed pose bytes. Template full-pose fallback remains a
proxy-only path.

`tools/braindance_pipeline.py` handles the next boundary. `link-scene`
rebuilds an authored rewindable scene's RID tables and playback-event
bindings; `link-quest` retargets a braindance questphase scene node;
`package` copies the resulting CR2W files to validated depot paths and records
their hashes. `runtime-init`, `runtime-record`, and `runtime-verify` maintain
hash-bound evidence for seek, rewind, three analysis layers, normal and
interrupted cleanup, and replay.
...
link commands, depot staging, and runtime evidence workflow.

The checked SQ012-derived scene, reusable `braindance_analysis` quest template,
typed `gqt005` test quest, journal, localization, and Kabuki world resources
...
Create neutral shells only when importing or replacing a vetted frame donor:

```powershell
py -B .\tools\character_builder.py make-shell `
  --donor .\characters\components\donors\npv-female.app.json `
...
`make-shell` removes all authored appearances. `make-entity-shell` preserves
the frame's engine component graph but reduces its appearance list to one
neutral mapping. Review and validate both outputs before changing a manifest to
use them.
...
py -B .\tools\character_builder.py `
  --manifest .\characters\female-example.character.json validate
py -B .\tools\character_builder.py `
  --manifest .\characters\female-example.character.json generate `
...
```powershell
py -B .\tools\character_builder.py `
  --manifest .\characters\female-example.character.json head `
...
```powershell
py -B .\tools\character_builder.py `
  --manifest .\characters\female-example.character.json preview `
...
catalog has a reviewed garment anchor.

Click `Prepare full character` to generate the current isolated appearance and
batch-export its morphable head, body, hair, and outfit silhouette into one
offline Three.js scene. The first run can take roughly a minute because it
packs a temporary character-only archive and uncooks all selected meshes;
unchanged runs reuse the fingerprinted GLBs. Use the `Head` and `Full character`
toolbar buttons to switch views after inspecting an individual asset. This
preview uses role-colored neutral materials and omits makeup/piercing overlays,
nails, and shadow proxies. It is a fit and silhouette check, not a substitute
...
structural Markdown references under `docs/reference/vanilla-quests`. Each
...
```powershell
py -B .\tools\quest_compiler.py compile `
  .\quests\examples\phone_conversation.quest.json `
...
```powershell
py -B .\tools\quest_compiler.py compile `
  .\quests\examples\direct_building_blocks.quest.json `
  --out .\converted\quest-blocks\gq_blocks_direct.questphase.json

py -B .\tools\quest_compiler.py compile `
  .\quests\examples\template_building_blocks.quest.json `
...
and local WolvenKit source assumptions. It does not use a root-level legacy
scene template,
...
locStore coverage, optional vanilla `icon_tags` on choice options, and the
scene-local quest wrappers used by
`gq000_patch_meet`.
...
See `docs/authoring/drop-points.md` for rebuild provenance, curation rules, and the
...
## Offline Item And Equipment Database

`tools/item_database.py` builds a localized SQLite/JSON wardrobe catalog from
the installed REDmod item records, player item-controller `.app` resources,
and the current character asset index. It can extract RED materials, render
studio images through headless Blender 5.1, serve a local searchable gallery,
and export JSONL caption-model jobs without launching the game.

See `docs/authoring/items-and-equipment.md` for the schema, cache boundaries,
commands, rendering limitations, and validation workflow.
