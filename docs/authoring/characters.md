...
profiles, neutral entity and appearance shells, tooling-owned component
libraries, separate vetted catalogs, a headless head build, live WebGL head
preview, installed-game asset index, and local web UI. Patch's reviewed design
manifest has been applied to the shipping raw/packed appearance and installed
for focused runtime validation. A checked female example can assemble the
tutorial's female component topology, stage its referenced assets under a
character-owned namespace, and build the PWA head. The original male and female
appearance documents remain component donors and semantic regression inputs;
they are not character templates. The temporary male shape-22 smoke test proved
the toolchain but was later shown to be a geometry no-op rather than Patch's
intended face; shape 22 is present and supported by the female PWA
morphtargets.
...
- the downloaded NPV WolvenKit template configured as the machine-local
  `@npv` root;
...
This makes the root entity a versioned, frame-specific shell rather than
character-authored data. The generator clones the vetted male or female shell,
replaces its one neutral appearance mapping and paths, and leaves the
engine-facing component graph intact.
...
### Shell and component-library boundary

The `.app` shell is deliberately empty. It owns only the
`appearanceAppearanceResource` container and serializer-level resource
metadata; it contains no appearance, outfit, or character choice. A manifest
does not select a donor appearance such as `casual` or `business`.
...
Donors contain only complete prototypes that the builder can select. The
...
them. For example, ordinary female manifests resolve to `base`; a manifest that
needs the extra garment components resolves to `extended_garments`. The chosen
prototype is copied into the empty shell, renamed for the character, mutated by
...
them to an empty resource would not be equivalent.

This split concerns CR2W-JSON authoring. Native JSON-to-CR2W writes are still
binary-template-backed: a packed character `.app` is usable only when it
already contains every class/layout instance required by the generated JSON.
An empty binary shell cannot teach the native writer how to encode a newly
populated appearance array, and Patch's current packed `.app` still lacks an
...
The base player-derived component prototype deliberately keeps its nude
...
2. Clone the pinned frame-specific root `.ent` shell and rewrite its neutral
   appearance mapping.
3. Select the smallest compatible component prototype, insert it into the empty
   `.app` shell, and apply reusable component bundles.
...
- `tools/character_builder.py` validates manifests, assembles empty shells from
  tooling-owned component prototypes, generates isolated source files, stages
  donor-owned assets, compares Patch with its handwritten baseline, and runs
  the complete WolvenKit -> Blender -> WolvenKit head build;
