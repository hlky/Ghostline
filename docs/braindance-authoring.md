...
`source/braindance/tests/gqt005_braindance_analysis.json`
...
py -B .\tools\braindance_scene.py validate `
  --spec .\source\braindance\tests\gqt005_braindance_analysis.json
...
- `source/braindance/rigs/man_base.glb`, a clean rig-only GLB with no mesh or
  imported animation actions; and
- `source/braindance/rigs/man_base.skeleton.json`, the exact 71-joint RED
...
{
  "asset": {
    "path": "source/braindance/rigs/man_base.glb"
  },
  "rig": {
    "contract": "source/braindance/rigs/man_base.skeleton.json"
  },
  "body_animation": {
...
For the checked test quest, `tools/generate_gqt005_content.py` performs the
